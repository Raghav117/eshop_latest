...
class _SignUpPageState extends State<SignUp> with TickerProviderStateMixin {
  ...
  Future<void> getRegisterUser() async {
    try {
      ...
      request.fields[FRNDCODE] = friendCode;
      request.fields["BusinessName"] = businessController.text;
      request.fields["GSTNumber"] = gstController.text;
      if (image != null) {
  ...
  subLogo() {
    return Expanded(
      flex: 2,
      child: Center(
        child: new Image.asset('assets/images/sliderph.png'),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        style:
            TextStyle(color: colors.fontColor, fontWeight: FontWeight.normal),
        validator: (val) =>
            validateUserName(val, "Business Name is Required", "Invalid"),
        // validator: (val) => validateEmail(
        //     val,
        //     getTranslated(context, 'EMAIL_REQUIRED'),
        //     getTranslated(context, 'VALID_EMAIL')),
        ...
        // },
        onFieldSubmitted: (v) {
          _fieldFocusChange(context, businessFocus, passFocus);
        },
        decoration: InputDecoration(
          prefixIcon: Icon(
            Icons.business,
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        style:
            TextStyle(color: colors.fontColor, fontWeight: FontWeight.normal),
        validator: (val) =>
            validateUserName(val, "GST Number is Required", "Invalid"),
        // validator: (val) => validateEmail(
        //     val,
        //     getTranslated(context, 'EMAIL_REQUIRED'),
        //     getTranslated(context, 'VALID_EMAIL')),
